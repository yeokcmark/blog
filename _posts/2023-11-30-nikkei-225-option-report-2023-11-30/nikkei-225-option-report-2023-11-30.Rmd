---
title: "Nikkei 225 Option Report 2023-11-30"
description: |
...
data_tpv_p <-
  data1 %>% 
  filter(X1 == "NK225E" & X2 == "OOP" & X3 %in% c("202312", "202401", "202402")) %>% 
...
dte_2312 <- bizdays(today(), "2023-12-08", "JPX_JAPAN") / 245
...
d_near_c <- 
  data_options %>%
  filter(type == "C" & contract == "2312")

d_near_p <- 
  data_options %>%
  filter(type == "P" & contract == "2312")
...
data_options <-
  data_options %>% 
  mutate(dte = as.numeric(ifelse(contract == "2312", dte_2312,
                      ifelse(contract == "2401", dte_2401, dte_2402)
                      )),
...
p_call_put_gamma <-
  ...
  scale_x_continuous(name = "Price",
                     limits = c(min(d$strike), max(d$strike)),
                     breaks = c(min(d$strike)+500*1:floor((max(d$strike)-min(d$strike))/500)),
...
p_opt_volume_near <-
  ...
  scale_x_continuous(name = "strike",
                     limits = c(min(d_near_volume$strike), max(d_near_volume$strike)),
                     breaks = c(min(d_near_volume$strike)+500*1:floor((max(d_near_volume$strike)-min(d_near_volume$strike))/500)),
                     labels = label_number(suffix = "K",
                                           scale = 1e-3))+
  scale_y_continuous(name = "volume")+
  labs(title = glue("Nikkei 225 Options Daily Traded Volume - {p_date}"),
       subtitle = glue("Contract: Dec 2023, Spot Price: {spot_price}"),
...
p_opt_volume_far <-
  ...
  geom_vline(xintercept = spot_price,
             ...
             color = "darkgreen")+
  scale_x_continuous(name = "strike",
                     limits = c(min(d_far_volume$strike), max(d_far_volume$strike)),
                     breaks = c(min(d_far_volume$strike)+500*1:floor((max(d_far_volume$strike)-min(d_far_volume$strike))/500)),
                     labels = label_number(suffix = "K",
                                           scale = 1e-3))+
  scale_y_continuous(name = "volume")+
  labs(title = glue("Nikkei 225 Options Daily Traded Volume - {p_date}"),
...
p_opt_change_OI_near <-
  ...
  geom_vline(xintercept = spot_price,
             ...
             color = "darkgreen")+
  scale_x_continuous(name = "strike",
                     limits = c(min(d_near_oic$strike), max(d_near_oic$strike)),
                     breaks = c(min(d_near_oic$strike)+1000*1:floor((max(d_near_oic$strike)-min(d_near_oic$strike))/1000)),
                     labels = label_number(suffix = "K",
                                           scale = 1e-3))+
  scale_y_continuous(name = "volume")+
...
p_opt_change_OI_far <-
  ...
  geom_vline(xintercept = spot_price,
             ...
             color = "darkgreen")+
  scale_x_continuous(name = "strike",
                     limits = c(min(d_far_oic$strike), max(d_far_oic$strike)),
                     breaks = c(min(d_far_oic$strike)+1000*1:floor((max(d_far_oic$strike)-min(d_far_oic$strike))/1000)),
                     labels = label_number(suffix = "K",
                                           scale = 1e-3))+
  scale_y_continuous(name = "volume")+
  labs(title = glue("Nikkei 225 Options Daily Change in Open Interest - {p_date}"),
...
p_opt_OI_current_near <-
  ...
  geom_vline(xintercept = spot_price,
             ...
             color = "darkgreen")+
  scale_x_continuous(name = "strike",
                     limits = c(min(d_near_toi$strike), max(d_near_toi$strike)),
                     breaks = c(min(d_near_toi$strike)+1000*1:floor((max(d_near_toi$strike)-min(d_near_toi$strike))/1000)),
                     labels = label_number(suffix = "K",
                                           scale = 1e-3))+
  scale_y_continuous(name = "volume")+
...
p_opt_OI_current_far <-
  ...
  geom_vline(xintercept = spot_price,
             ...
             color = "darkgreen")+
  scale_x_continuous(name = "strike",
                     limits = c(24000, max(d_far_toi$strike)),
                     breaks = c(23000+1000*1:floor((max(d_far_toi$strike)-min(d_far_toi$strike))/1000)),
                     labels = label_number(suffix = "K",
                                           scale = 1e-3))+
  scale_y_continuous(name = "volume")+
  labs(title = glue("Nikkei 225 Options Daily Open Interest - {p_date}"),
...
Date: `r {p_date}` Contract: Dec 2023\
...
```{r, layout="l-body-outset", warning=FALSE, preview = TRUE}
p_total_gamma
```
