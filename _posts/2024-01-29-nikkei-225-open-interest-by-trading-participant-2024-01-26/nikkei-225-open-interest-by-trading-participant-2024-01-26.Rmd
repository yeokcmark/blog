---
title: "Nikkei 225 Open Interest by Trading Participant 2024-01-26"
date: 2024-01-29
...
data_p_ns<-
  ...
  select(1,3,4) %>%
  drop_na() %>%
  rename(strike = "PUT February 2024",
         participant_name = "...4",
         volume = "...5") %>%
...
data_c_ns<-
  ...
  select(1,3,4) %>%
  drop_na() %>%
  rename(strike = "CALL February 2024",
         participant_name = "...14",
         volume = "...15") %>%
  mutate(volume = -1*volume,
         type = "C")
...
# Since this is week one for 2311, data_23_11 is the same as data_weekly
 # data_2402 <- data_weekly
 # saveRDS(data_2402, file = "data_2402.RDS")
#
#
data_2402<-
  readRDS (file = "data_2402.RDS")
...
#for week 2, 3, 4, we need to merge and find new net position
 ...
 d <-
   full_join (data_2402, data_weekly, by = c("strike", "type", "participant_name")) %>%
...
# Lets consider data for index futures
f_file_name <- glue("{download_date}_indexfut_oi_by_tp_e.xlsx")
#GET(URL, write_disk(file_name, overwrite=TRUE))

f_data <- read_excel(f_file_name,
                   sheet = "BO_DD0121",
                   skip = 3)
names(f_data)
