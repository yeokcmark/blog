...
# grid_num_clusters <- grid_regular(num_clusters(),levels = 10)
...
Alternatively, we can do it the Base R way.


```{r}
# rec_km <-
#   recipe(formula = ~ fwd_pkts_payload_min + flow_iat_min + service + 
#            fwd_subflow_bytes + fwd_last_window_size,
#          data = data) %>% 
#   # update_role(id, network_attack, proto, service, attack_type,
#   #             id_orig_p, id_resp_p, new_role = "id") %>% 
#   step_nzv(all_predictors())
